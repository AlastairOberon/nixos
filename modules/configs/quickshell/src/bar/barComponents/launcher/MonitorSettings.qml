...
Item {
    ...
    function applyQuickshellScale(monitorName, scaleValue) {
        ...
        let scalesJson = JSON.stringify(Theme.monitorScales);
        
        let script = [
            'FILE="$HOME/.cache/quickshell_JellyfishDungeon.json"',
            'if [ ! -f "$FILE" ] || ! jq . "$FILE" >/dev/null 2>&1; then echo \'{}\' > "$FILE"; fi',
            'jq --argjson scales \'' + scalesJson + '\' \'.monitorScales = $scales\' "$FILE" > "$FILE.tmp" && mv "$FILE.tmp" "$FILE"',
            '~/.config/quickshell/assets/launch_quickshell.sh'
        ].join('\n');

        saveConfigProcess.running = false;
