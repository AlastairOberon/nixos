...
Item {
    ...
    property int tick: 0 
    property bool is24HourFormat: Settings.is24HourFormat
    ...
    function loadSavedSettings() {
        timeDateRoot.is24HourFormat = Settings.is24HourFormat;
        let zones = Settings.activeZones;
        if (zones && zones.length > 0) {
            activeZonesModel.clear();
            for (let i = 0; i < zones.length; i++) {
                activeZonesModel.append(zones[i]);
            }
        }
    }
    ...
    function saveSettings() {
        ...
        for (let i = 0; i < activeZonesModel.count; i++) {
            ...
            zonesArr.push({
                ...
                lat: z.lat || 0,
                lon: z.lon || 0,
                offset: z.offset !== undefined ? z.offset : 0
            });
        }
        Settings.setTimeDateSettings(timeDateRoot.is24HourFormat, zonesArr);
    }

    Component.onCompleted: {
        timeDateRoot.loadSavedSettings();
    }
